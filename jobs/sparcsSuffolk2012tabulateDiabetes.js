...
sparcsSuffolk2012.url=[
	"https://health.data.ny.gov/resource/u4ud-w55t.json?hospital_county=Suffolk&ccs_diagnosis_code=186",
	"https://health.data.ny.gov/resource/u4ud-w55t.json?hospital_county=Suffolk&ccs_diagnosis_code=050",
	"https://health.data.ny.gov/resource/u4ud-w55t.json?hospital_county=Suffolk&ccs_diagnosis_code=049"
	]
...
s.loadData(function(x){
	...
	$("#openHealthJob").append('<hr><div id="allPatients"><h3>All Diabetes patients</h3></div></hr><hr><div id="medicaidPatients"><h3>Medicaid Diabetes patients</h3></div></hr>');
	...
	bt.onclick=function(){
		$('#allPatients').html("<h3>All Diabetes patients ("+s.docs.length+")</h3>")
		$('#medicaidPatients').html("<h3>Medicaid Diabetes patients ("+s.medicaid.docs.length+")</h3>")
		...
		$('#allPatients').append(openHealth.crossdoc2html(
			...
			'counts of '+selParm1+' vs '+selParm2+' for all Diabetes patients.csv'
		));
		$('#medicaidPatients').append(openHealth.crossdoc2html(
			openHealth.tabulateCount(s.medicaid.tab,selParm1,selParm2,Up1,Up2),
			'counts of '+selParm1+' vs '+selParm2+' for Medicaid Diabetes patients.csv'
		));
