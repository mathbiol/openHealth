...
openHealth.require('https://mathbiol.github.io/openHealth/tcga/tcga.js',function(){
	var selectTumorHTML='<h3 style="color:navy">Tumor Type: <select onchange="tumorChanged(this)" style="font-color:navy;background-color:silver;font-size:large" id="selectTumor"><option>GBM - Glioblastoma Multiforme</option><option>LGG - Lower Grade Glioma</option><option>LUAD - Lung Cancer</option></select></h3>'
            		...
            		*/
            		pp.map(function(p){
            			if(!document.getElementById("link_"+p)){
            				...
            				dx.id="link_"+p
            				dx.innerHTML='<a href="http://quip.bmi.stonybrook.edu/camicroscope-qin/osdCamicroscope.php?tissueId='+p+'" target=_blank>'+p+'</a>'
            				tp.appendChild(dx)
            			}
    ...
    get_biospecimen_slide_luad=function(){
        //localforage.removeItem('biospecimen_slide_gbm')
        localforage.getItem('biospecimen_slide_luad',function(x){
            if(!x){
                openHealth.tcga.getTable("luad/bcr/biotab/clin/nationwidechildrens.org_biospecimen_slide_luad.txt",
                function(x){
                    openHealth.tcga.dt['biospecimen_slide_luad']=x
                    localforage.setItem('biospecimen_slide_luad',x)
                    console.log('biospecimen_slide_luad loaded from TCGA and cached for this machine')
                    LUADfun2()
                },
                0,
                2
                ...
                console.log('biospecimen_slide_luad retrieved from cache')
                openHealth.tcga.dt['biospecimen_slide_luad']=x
                LUADfun2()
            }
    ...
    if(!openHealth.tcga.iniTumorSelected){
    	openHealth.tcga.iniTumorSelected="LUAD"
    	setTimeout(function(){
    		selectTumor.selectedIndex=2
    		tumorChanged(selectTumor)
    	},2000)
    }
    ...
    // --> LUAD <--
	doLUAD=function(){
	localforage.getItem('clinical_patient_luad',function(x){
		//localforage.removeItem('clinical_patient_gbm')
		if(!x){
			openHealth.tcga.getTable("luad/bcr/biotab/clin/nationwidechildrens.org_clinical_patient_luad.txt",
			  function(x){
				openHealth.tcga.dt['clinical_patient_luad']=x
				localforage.setItem('clinical_patient_luad',x)
				console.log('clinical_patient_luad loaded from TCGA and cached for this machine')
				get_biospecimen_slide_luad()
			  },
			  1,
			  3
			)
		} else{
			console.log('clinical_patient_luad retrieved from cache')
			openHealth.tcga.dt['clinical_patient_luad']=x
			get_biospecimen_slide_luad()
		}
    ...
    var LUADfun=function(){
        // Extract AGE
        openHealth.tcga.dt.clinical_patient_luad.age=openHealth.tcga.dt.clinical_patient_luad.age_at_initial_pathologic_diagnosis.map(function(xi){
            return parseInt(xi)
        })
        // Extract SURVIVAL
        openHealth.tcga.dt.clinical_patient_luad.survival=openHealth.tcga.dt.clinical_patient_luad.days_to_death.map(function(xi,i){
            if(xi=="[Not Applicable]"){
                xi = openHealth.tcga.dt.clinical_patient_luad.days_to_last_followup[i] // this is not ideal so we'll need to flag the vital status in teh analysis
            }
            return parseInt(xi)
        })
        openHealth.tcga.dt.clinical_patient_luad.dead=openHealth.tcga.dt.clinical_patient_luad.vital_status.map(function(xi){
            return xi=="Dead"
        })
        openHealth.tcga.dt.clinical_patient_luad.score=openHealth.tcga.dt.clinical_patient_luad.karnofsky_performance_score.map(function(xi,i){
            if(!parseFloat(xi)){return NaN}
            else{return parseInt(xi)}	// karnofsky_performance_score
        })
        ...
        // Create Docs
        var docs = openHealth.tab2docs(openHealth.tcga.dt.biospecimen_slide_luad) // one doc per image
        // index patients by bcr code
        var patient = {}
        openHealth.tab2docs(openHealth.tcga.dt.clinical_patient_luad).forEach(function(xi){
            patient[xi.bcr_patient_barcode]=xi
        })
        // add patient info to slide docs
        docs=docs.map(function(xi,i){
        ...
        docs=d;
        openHealth.tcga.dt.luadDocs=docs
        openHealth.tcga.dt.luadTab=openHealth.docs2tab(openHealth.tcga.dt.luadDocs)
        //openHealthJob.innerHTML='<div id="openHealthJobMsg" style="color:red">TCGA Data retrieved, processing it ...</div>'+selectTumorHTML+'<div id="openHealthJobDC"></div>'
        openHealthJobMsg.textContent='--> processing ...'
        openHealthJobDC.innerHTML=''
        ...
        openHealth.getScript(["https://cdnjs.cloudflare.com/ajax/libs/d3/3.4.11/d3.min.js","https://www.google.com/jsapi","https://square.github.io/crossfilter/crossfilter.v1.min.js","https://dc-js.github.io/dc.js/js/dc.js","https://dc-js.github.io/dc.js/css/dc.css"],function(){ // after satisfying d3 dependency
            openHealthJobMsg.textContent="Assembling charts ..."
            openHealthJobDC.innerHTML='<table cellpadding="10px"><tr><td style="vertical-align:top"><table><tr><td style="vertical-align:top"><div>% Necrotic Cells:</div><div id="percent_necrosis"></div><div>% Tumor Nuclei:</div><div id="percent_tumor_nuclei"></div><div>% Neutrophil Infiltration:</div><div id="percent_neutrophil_infiltration"></div><div>Location:</div><div id="section_location"></div></td><td style="vertical-align:top"><div>% Tumor Cells:</div><div id="percent_tumor_cells"></div><div>% Lymphocyte Infiltration:</div><div id="percent_lymphocyte_infiltration"></div><div>Race:</div><div id="race"></div><div>Gender:</div><div id="gender"></div></td><td style="vertical-align:top"><div>% Stromal Cells:</div><div id="percent_stromal_cells"></div><div style="color:blue">Karnofsky Score:</div><div id="karnofsky_performance_score" style="border:solid;border-color:blue;box-shadow:10px 10px 5px #888888"></div><div>% Monocyte Infiltration:</div><div id="percent_monocyte_infiltration"></div></td></tr></table></td><td style="vertical-align:top"><h3>LUAD Tumor progression</h3><div id="tumorProgression"></div><b>Legend</b>: color indicates Karnofsky performance score (see framed bar chart); diameter indicates number of images</td></tr></table><table><tr><td style="vertical-align:top"><div id="tcgaPatientsHeader">TCGA patients:</div><div id="tcgaPatients"></div></td><td style="vertical-align:top"><div id="slideImagesHeader">Slide Images:</div><div id="slideImages"></div></td><td style="vertical-align:top"><div id="diagnosticImagesHeader">Diagnostic Images:</div><div id="diagnosticImages"></div></td><td style="vertical-align:top"><div id="buttonResults"></div></td></tr></table>'
            var docs = openHealth.tcga.dt.luadDocs
            var tab = openHealth.tcga.dt.luadTab

            var C = {}, D={}, G={}, U={}, R={}
            var P={},S={}  // list patients and slides
            var listDxSlides=function(pp){
            	// check DxImages available already
            	if(!openHealth.tcga.dt.luadDx){
            		openHealth.getText('https://sbu-bmi.github.io/appliedApps/luad_patientids.json',function(x){
            			x=x.replace(/}/g,'},')
            			x='['+x.slice(0,-2)+']'
            			x=JSON.parse(x.replace(/\'/g,'"'))
            			var y = {} // index of diagnostic images per patient
            			...
            			openHealth.tcga.dt.luadDx=y
            			listDxSlides(pp)
            		})
            	}else{
            		var pp0=pp.filter(function(pi){return openHealth.tcga.dt.luadDx[pi]})
            		pp=[]
            		pp0.map(function(pi){
            			pp=pp.concat(openHealth.tcga.dt.luadDx[pi])
            		})
            		diagnosticImagesHeader.textContent=' Diagnostic Images ('+pp.length+'):'
            		diagnosticImages.innerHTML="" // clear
            		pp.map(function(pi){
            			//var di = openHealth.tcga.dt.luadDx[pi]
            			var a = document.createElement('a')
            			a.href="http://quip.bmi.stonybrook.edu/camicroscope-qin/osdCamicroscope.php?tissueId="+pi
            			a.textContent=pi
            			a.target="_blank"
			...
			var listSlides=function(){
				...
				openHealth.tcga.resultsSlide=function(x){
					var d = openHealth.findOne(openHealth.tcga.dt.luadDocs,'bcr_slide_barcode',x.textContent)
					buttonResults.innerHTML='<pre>'+JSON.stringify(d,null,3)+'</pre>'
				}
				pp.sort().forEach(function(p,i){
					var pr = document.createElement('p')
					pr.innerHTML=' '+i+') <button onclick="openHealth.tcga.resultsPatient(this)">'+p+'</button> <a href="http://www.cbioportal.org/case.do?case_id='+p+'&cancer_study_id=luad_tcga" target=_blank>cBio</a>. '
					tcgaPatients.appendChild(pr)
				})
				ss.sort().forEach(function(s,i){
					var pr = document.createElement('p')
            ...
            R[parm]={}
            P[parm]={}
            openHealth.unique(openHealth.tcga.dt.luadTab.patient).map(function(p){
            	P[parm][p]={c:0}
            })
            S[parm]={}
			openHealth.tcga.dt.luadTab.bcr_slide_barcode.map(function(s){
            	S[parm][s]={c:0}
            })
			...
			addRowChard('percent_monocyte_infiltration')
			addRowChard('percent_neutrophil_infiltration')
			addRowChard2('section_location',openHealth.unique(openHealth.tcga.dt.luadTab.section_location))
			addRowChard2('gender',openHealth.unique(openHealth.tcga.dt.luadTab.gender))
			addRowChard('race',openHealth.unique(openHealth.tcga.dt.luadTab.race))
			addRowChard(
				'karnofsky_performance_score',
				openHealth.unique(openHealth.tcga.dt.luadTab.karnofsky_performance_score),
				function(CRT){
					CRT
						.colors(d3.scale.linear().domain([-1,0,40,80,90,100]).range(["silver","red","red","yellow","green","green"]))
						.colorAccessor(function (d,i) {
			...
			D.tumorProgression = cf.dimension(function(d){return d.patient})
			R.tumorProgression={}
			openHealth.unique(openHealth.tcga.dt.luadTab.patient).map(function(u){
				R.tumorProgression[u]={c:0}
			})
			G.tumorProgression = D.tumorProgression.group().reduce(
				// reduce in
			  ...
			  function(){return 0}
       		...
       		C.tumorProgression
       			...
       			.radiusValueAccessor(function (v) {
       				...
       				if(v.value>0){
       					var p = document.createElement('p')
						p.innerHTML=tcgaPatients.children.length+1+') <a href="http://www.cbioportal.org/case.do?case_id='+v.key+'&cancer_study_id=luad_tcga" target=_blank>'+v.key+'</a>'
						tcgaPatients.appendChild(p)
       				}
       				*/
    ...
    var LUADfun2=function(){
        ...
        openHealth.getScript(["https://cdnjs.cloudflare.com/ajax/libs/d3/3.4.11/d3.min.js","https://www.google.com/jsapi","https://square.github.io/crossfilter/crossfilter.v1.min.js","https://dc-js.github.io/dc.js/js/dc.js","https://dc-js.github.io/dc.js/css/dc.css"],function(){ // after satisfying d3 dependency
            openHealthJobMsg.textContent="Assembling charts ..."
            //openHealthJobDC.innerHTML='<table cellpadding="10px"><tr><td style="vertical-align:top"><table><tr><td style="vertical-align:top"><div>% Necrotic Cells:</div><div id="percent_necrosis"></div><div>% Tumor Nuclei:</div><div id="percent_tumor_nuclei"></div><div>Location:</div><div id="section_location"></div></td><td style="vertical-align:top"><div>% Tumor Cells:</div><div id="percent_tumor_cells"></div><div>% Lymphocyte Infiltration:</div><div id="percent_lymphocyte_infiltration"></div><div>Race:</div><div id="race"></div><div>Gender:</div><div id="gender"></div></td><td style="vertical-align:top"><div>% Stromal Cells:</div><div id="percent_stromal_cells"></div><div style="color:blue">Karnofsky Score:</div><div id="karnofsky_performance_score" style="border:solid;border-color:blue;box-shadow:10px 10px 5px #888888"></div><div>% Monocyte Infiltration:</div><div id="percent_monocyte_infiltration"></div><div>% Neutrophil Infiltration:</div><div id="percent_neutrophil_infiltration"></div></td></tr></table></td><td style="vertical-align:top"><h3>luad Tumor progression</h3><div id="tumorProgression"></div><b>Legend</b>: color indicates Karnofsky performance score (see framed bar chart); diameter indicates number of images</td></tr></table><table><tr><td style="vertical-align:top"><div id="tcgaPatientsHeader">TCGA patients:</div><div id="tcgaPatients"></div></td><td style="vertical-align:top"><div id="slideImagesHeader">Slide Images:</div><div id="slideImages"></div></td><td style="vertical-align:top"><div id="diagnosticImagesHeader">Diagnostic Images:</div><div id="diagnosticImages"></div></td><td style="vertical-align:top"><div id="buttonResults"></div></td></tr></table>'
            //openHealthJobDC.innerHTML='<table cellpadding="10px"><tr><td style="vertical-align:top"><table><tr><td style="vertical-align:top"><div>% Necrotic Cells:</div><div id="percent_necrosis"></div><div>% Tumor Nuclei:</div><div id="percent_tumor_nuclei"></div><div>Location:</div><div id="section_location"></div></td><td style="vertical-align:top"><div>% Tumor Cells:</div><div id="percent_tumor_cells"></div><div>% Lymphocyte Infiltration:</div><div id="percent_lymphocyte_infiltration"></div><div>Race:</div><div id="race"></div><div>Gender:</div><div id="gender"></div></td><td style="vertical-align:top"><div>% Stromal Cells:</div><div id="percent_stromal_cells"></div><div style="color:blue">Karnofsky Score:</div><div id="karnofsky_performance_score" style="border:solid;border-color:blue;box-shadow:10px 10px 5px #888888"></div><div>% Monocyte Infiltration:</div><div id="percent_monocyte_infiltration"></div><div>% Neutrophil Infiltration:</div><div id="percent_neutrophil_infiltration"></div></td></tr></table></td><td style="vertical-align:top"><h3>luad Tumor progression</h3><div id="tumorProgression"></div><b>Legend</b>: color indicates Karnofsky performance score (see framed bar chart); diameter indicates number of images</td></tr></table><table id="patientSlideTable"><thead><tr><td id="tcgaPatientsHeader">TCGA patients:</td><td id="diagnosticImagesHeader">Diagnostic Images:</td></tr></thead><tr><td style="vertical-align:top"><div id="tcgaPatientsHeader_">TCGA patients:</div><div id="tcgaPatients"></div></td><td style="vertical-align:top"><div id="slideImagesHeader">Slide Images:</div><div id="slideImages"></div></td><td style="vertical-align:top"><div id="diagnosticImagesHeader_">Diagnostic Images:</div><div id="diagnosticImages"></div></td><td style="vertical-align:top"><div id="buttonResults"></div></td></tr></table>'
            //openHealthJobDC.innerHTML='<table cellpadding="10px"><tr><td style="vertical-align:top"><table><tr><td style="vertical-align:top"><div>% Necrotic Cells:</div><div id="percent_necrosis"></div><div>% Tumor Nuclei:</div><div id="percent_tumor_nuclei"></div><div>Location:</div><div id="section_location"></div></td><td style="vertical-align:top"><div>% Tumor Cells:</div><div id="percent_tumor_cells"></div><div>% Lymphocyte Infiltration:</div><div id="percent_lymphocyte_infiltration"></div><div>Race:</div><div id="race"></div><div>Gender:</div><div id="gender"></div></td><td style="vertical-align:top"><div>% Stromal Cells:</div><div id="percent_stromal_cells"></div><div style="color:blue">Karnofsky Score:</div><div id="karnofsky_performance_score" style="border:solid;border-color:blue;box-shadow:10px 10px 5px #888888"></div><div>% Monocyte Infiltration:</div><div id="percent_monocyte_infiltration"></div><div>% Neutrophil Infiltration:</div><div id="percent_neutrophil_infiltration"></div></td></tr></table></td><td style="vertical-align:top"><h3>luad Tumor progression</h3><div id="tumorProgression"></div><b>Legend</b>: color indicates Karnofsky performance score (see framed bar chart); diameter indicates number of images</td></tr></table><table id="patientSlideTable"><thead><tr><td id="tcgaPatientsHeader" style="color:maroon;font-weight:bold">TCGA patients:</td><td id="diagnosticImagesHeader" style="color:maroon;font-weight:bold">Diagnostic Images:</td></tr></thead><tbody id="patientSlideTableBody"></tbody></table><table id="hiddenTable" hidden=true><tr><td style="vertical-align:top"><div id="tcgaPatientsHeader_">TCGA patients:</div><div id="tcgaPatients"></div></td><td style="vertical-align:top"><div id="slideImagesHeader">Slide Images:</div><div id="slideImages"></div></td><td style="vertical-align:top"><div id="diagnosticImagesHeader_">Diagnostic Images:</div><div id="diagnosticImages"></div></td><td style="vertical-align:top"><div id="buttonResults"></div></td></tr></table>'
            openHealthJobDC.innerHTML='<table cellpadding="10px"><tr><td style="vertical-align:top"><table><tr><td style="vertical-align:top"><div>% Necrotic Cells:</div><div id="percent_necrosis"></div><div>% Tumor Nuclei:</div><div id="percent_tumor_nuclei"></div><div>% Neutrophil Infiltration:</div><div id="percent_neutrophil_infiltration"></div><div>Location:</div><div id="section_location"></div></td><td style="vertical-align:top"><div>% Tumor Cells:</div><div id="percent_tumor_cells"></div><div>% Lymphocyte Infiltration:</div><div id="percent_lymphocyte_infiltration"></div><div>Gender:</div><div id="gender"></div></td><td style="vertical-align:top"><div>% Stromal Cells:</div><div id="percent_stromal_cells"></div><div style="color:blue">Karnofsky Score:</div><div id="karnofsky_performance_score" style="border:solid;border-color:blue;box-shadow:10px 10px 5px #888888"></div><div>% Monocyte Infiltration:</div><div id="percent_monocyte_infiltration"></div><div>Race:</div><div id="race"></div></td></tr></table></td><td style="vertical-align:top"><h3>LUAD Tumor progression</h3><div id="tumorProgression"></div><b>Legend</b>: color indicates Karnofsky performance score (see framed bar chart); diameter indicates number of images</td></tr></table><table><tr><td style="vertical-align:top"><table id="patientSlideTable"><thead><tr><td id="tcgaPatientsHeader" style="color:maroon;font-weight:bold">TCGA patients:</td><td id="diagnosticImagesHeader" style="color:maroon;font-weight:bold">Diagnostic Images:</td></tr></thead><tbody id="patientSlideTableBody"></tbody></table></td><td id="moreInfo" style="vertical-align:top"></td></tr></table><table id="hiddenTable" hidden=true><tr><td style="vertical-align:top"><div id="tcgaPatientsHeader_">TCGA patients:</div><div id="tcgaPatients"></div></td><td style="vertical-align:top"><div id="slideImagesHeader">Slide Images:</div><div id="slideImages"></div></td><td style="vertical-align:top"><div id="diagnosticImagesHeader_">Diagnostic Images:</div><div id="diagnosticImages"></div></td><td style="vertical-align:top"><div id="buttonResults"></div></td></tr></table>'
			...
			var listSlides=function(){
				...
				pp.sort().forEach(function(p,i){
					...
					var tr = document.createElement('tr')
					tr.id='tr_'+p
					tr.innerHTML='<td id="tdPatient_'+p+'" style="vertical-align:top">'+i+') <button onclick="openHealth.tcga.resultsPatient(this)">'+p+'</button>(<a href="http://www.cbioportal.org/case.do?case_id='+p+'&cancer_study_id=luad_tcga" target=_blank>cBio</a>)</td><td id="dxSlide_'+p+'" style="vertical-align:top;font-size:12"></td>'
					patientSlideTableBody.appendChild(tr)
					/*if(openHealth.tcga.dt.luadDx[p]){
						openHealth.tcga.dt.luadDx[p].map(function(dxi){
							var c=document.createElement('span')
							c.textContent=dxi
							pr.appendChild(c)
						})
